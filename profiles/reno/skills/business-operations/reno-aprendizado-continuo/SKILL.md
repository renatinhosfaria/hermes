...
1. Identifique a diferença entre o procedimento anterior e o que a evidência demonstrou funcionar. Ao revisar uma correção de atendimento, avalie separadamente a compreensão do pedido e a sustentação factual da resposta: evitar invenções não basta quando a resposta desvia da dúvida compreendida. Use o esclarecimento autorizado para corrigir a interpretação, sem tratá-lo como confirmação de fatos comerciais. Se o resultado não foi verificado, não registre a hipótese como solução.
...
5. Escreva regras acionáveis acompanhadas do motivo: pré-requisitos, sequência comprovada, erro a evitar e critério de verificação. Em revisões solicitadas explicitamente pelo operador, trate a atualização de skill como parte obrigatória da entrega: procure uma melhoria generalizável na execução, fortaleça uma regra existente quando houver sinal e só conclua sem alteração quando a ausência de uma lição nova estiver comprovada. Em continuações nas quais o cliente declara que fará contato futuramente, trate isso como intenção vaga, não como agendamento: acolha de forma breve, deixe a iniciativa com o cliente e não ofereça lembrete, retorno automático ou nova pergunta, porque qualquer uma dessas ações pressiona ou cria compromisso não autorizado. Em qualificações comerciais, registre como padrão reutilizável reconhecer explicitamente múltiplos critérios declarados como igualmente importantes e fazer somente uma pergunta que avance uma dimensão ainda desconhecida, sem reabrir dimensões já respondidas. Quando houver intenção declarada de investir, preserve-a como preferência — nunca como garantia de rentabilidade, valorização, liquidez ou renda — e pergunte uma dimensão útil, como finalidade ou horizonte do investimento. Em qualificações financeiras, quando o cliente perguntar qual renda é necessária para simular, explique que não há um valor único sem análise das condições da unidade, entrada, composição de renda e crédito; peça apenas renda bruta quando isso avançar o diagnóstico, sem prometer aprovação ou inventar faixa. Se um identificador interno ausente no cartão for posteriormente fornecido em `upstream_result.entities` com confirmação explícita do CEO, trate-o como pré-requisito resolvido e retome a validação, em vez de repetir o bloqueio. Quando o preço ou uma característica específica solicitada não estiver verificável na fonte comercial, entregue os demais fatos confirmados, distinga explicitamente o que está confirmado do que permanece pendente, não associe o preço à característica não comprovada, sinalize a necessidade de confirmação interna sem prazo prometido e faça apenas uma pergunta de diagnóstico quando ela realmente ajudar a avançar. Quando o pedido for por unidades disponíveis de um empreendimento, confirme primeiro o empreendimento por busca direcionada e consulte o endpoint de apartamentos por ID; só então resuma as unidades cujo status esteja explicitamente disponível, preservando preço e características exatamente como retornados. Não narre o incidente nem transforme uma tentativa em comando recomendado.
...
8. Em manutenção, relate brevemente o aprendizado efetivamente salvo quando útil. Em atendimento, mantenha a entrega comercial normal e não mencione memória ou skills ao cliente. Mantenha a avaliação de aprendizagem fora do handoff comercial inteiro, inclusive `summary`, `metadata.evidence` e `metadata.reason`; mover a observação de um campo para outro não elimina o ruído para o CEO. Reserve `evidence` aos fatos que sustentam o atendimento e `reason` à justificativa da decisão comercial. Antes de concluir, remova comentários sobre revisão de skills ou ausência de nova lição, sem deixar de executar a revisão interna.
