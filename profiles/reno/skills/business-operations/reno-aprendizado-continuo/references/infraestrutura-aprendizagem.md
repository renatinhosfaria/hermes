...
4. Distinga os mecanismos: o fork nativo de revisão só dispara após resposta final não interrompida, quando acionado, e depende das ferramentas disponíveis; não presuma revisão a cada mensagem nem capacidade de escrita apenas pelos intervalos. O Curator periódico mantém o ciclo de vida das skills. Não use `curator.consolidate=false` como diagnóstico de aprendizagem desligada: essa opção controla consolidação, não substitui nem desativa o fork pós-turno.
...
- Verifique a espera da revisão no encerramento real do CLI one-shot, não somente no script de teste; a thread daemon `bg-review` pode morrer com o processo antes de persistir. No plugin local `fama-learning-lifecycle`, preserve a espera pelo hook oficial `on_session_finalize`, antes do cleanup, restrita ao CLI e limitada a 300 segundos; mantenha canais gateway assíncronos. Valide separadamente término da revisão, isolamento de canal e limite de espera. Atingir o limite não comprova conclusão nem gravação; exija recuperação em processo independente. Use o hook local sem alterar a instalação do Hermes.
