...
| `AGENTS.md` / `agents.md` | CWD at startup + progressively discovered nested copies | You want portable agent instructions that work the same in Hermes, Claude Code, Codex, etc. |
...
- **Use `AGENTS.md`** when the same project will also be worked on by other agents (Codex, Claude Code, OpenCode). Those tools all have their own conventions for `AGENTS.md`; keep the file portable and place more specific overrides in nested directories when needed.
